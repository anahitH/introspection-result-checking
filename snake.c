...
int calc_score_level(int score, int level) {
  return score + level * 1000;
}

int calc_usec_delay(int delay, int level) {
   return delay - level * 10000;
}
...
void setup_level (screen_t *screen, snake_t *snake, int level)
{
   ...
   {
      screen->score = calc_score_level(screen->score, screen->level);
      screen->obstacles += 2;
   ...
   usec_delay = calc_usec_delay(DEFAULT_DELAY, level);
...
int get_next_dir(char key, int prev) {
...
void move (snake_t *snake, char key)
{
   int i;
   ...
   */
   snake->dir = get_next_dir(key, prev);
...
{
   snake_segment_t *head = &snake->body[snake->len - 1];
   return collide_walls (head->row, head->col) ||
      collide_object (snake, screen, CACTUS) ||
      collide_self (snake);
}

int calc_score_gold(int score, int len, int obstacles) {
   return score + len * obstacles;
}
int score_max(int score, int high_score) {
   if (score > high_score)
      return score;
   return high_score;
}
...
int eat_gold (snake_t *snake, screen_t *screen)
{
   ...
   screen->score = calc_score_gold(screen->score, snake->len, screen->obstacles);
   snake->len++;

   screen->high_score = score_max(screen->score, screen->high_score); //screen->score; /* New high score! */
